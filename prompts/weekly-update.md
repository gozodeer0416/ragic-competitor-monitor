...
2. 用 WebSearch 補漏：搜尋「{競品名} + 新功能／pricing／announcement／AI」等組合，涵蓋官方來源沒列到的重大新聞（募資、併購、重大改版、市場動作）。Kintone 需額外用日文關鍵字搜尋。
   - 若該競品在 config 有 `watch_keywords`（目前 Airtable：Hyperagent／Omni／Field Agents／AI automation；monday.com：Vibe／Agent Factory／Sidekick／AI workflow）：**額外逐一用「{競品名} + {關鍵字}」搜尋這些指定字詞有無變動**（新功能、改名、定價調整、棄用等），不能只靠通用搜尋帶到就算，要主動查。
...
- **conclusion**：從本週所有訊號歸納一個跨競品的主軸敘事，不是流水帳。`body` 為純文字但**視內容長度分段與列點**：不同主題之間用空行（`\n\n`）分段；並列的訊號或要點用「`- `」開頭的行列點（前端會自動渲染成段落與 bullet list）。短結論可維持單段，不要為了列點而列點。
